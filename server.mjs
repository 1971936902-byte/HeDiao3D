...
const server = createServer(async (req, res) => {
  try {
    ...
    const orchestratorLinuxCamJobPreflightMatch = req.url?.match(/^\/api\/orchestrator\/jobs\/([^/?#/]+)\/linux-cam-job-preflight$/);
    if (req.method === "POST" && orchestratorLinuxCamJobPreflightMatch) {
    ...
    const orchestratorCamoticsExecutionPreflightMatch = req.url?.match(/^\/api\/orchestrator\/jobs\/([^/?#/]+)\/camotics-execution-preflight$/);
    if (req.method === "POST" && orchestratorCamoticsExecutionPreflightMatch) {
      return createOrchestratorCamoticsExecutionPreflight(orchestratorCamoticsExecutionPreflightMatch[1], res);
    }
  ...
  const layers = [
    ...
    createV3GoalAuditLayer({
      ...
      evidence: [
        ...
        latestJob?.camoticsExecutionPreflight?.artifactExists ? `CAMotics执行预检: ${latestJob.camoticsExecutionPreflight.status ?? "unknown"} / currentHost=${latestJob.camoticsExecutionPreflight.canRunOnCurrentHost ? "ready" : "not-ready"}` : "缺少 CAMotics 执行预检",
      ...
      missing: [
        ...
        ...(!latestJob?.camoticsExecutionPreflight?.artifactExists ? ["缺少当前 job 的 CAMotics 执行预检报告"] : []),
...
function createV3DeploymentAcceptancePlan({ gates, diagnostics, nativeCam, adapterValidation, nativeCamRealOutputAcceptance, runbookResult, camServerConfig, externalHandoff, externalCamHandoffs, neutralImport, camoticsImport, readinessCamoticsEvidence, latestJob, latestTrialFeedback, latestMachineAcceptance, latestEvidenceDossier }) {
  ...
  const steps = [
    ...
    createAcceptanceStep({
      order: 14,
      id: "camotics-execution-preflight",
      title: "CAMotics 执行预检",
      status: !latestJob
        ? "pending"
        : !latestJob.camoticsExecutionPreflight?.artifactExists
          ? "pending"
          : latestJob.camoticsExecutionPreflight.status === "material-removal-result-imported" || latestJob.camoticsExecutionPreflight.status === "ready-to-run-current-host"
            ? "done"
            : "blocked",
      command: "POST /api/orchestrator/jobs/:id/camotics-execution-preflight",
      evidence: ["camotics-execution-preflight.json", "camotics-execution-preflight.md", "camotics-cli-run-package.json"],
      detail: latestJob?.camoticsExecutionPreflight?.artifactExists
        ? `${latestJob.camoticsExecutionPreflight.status ?? "unknown"} / currentHost=${latestJob.camoticsExecutionPreflight.canRunOnCurrentHost ? "ready" : "not-ready"} / command=${latestJob.camoticsExecutionPreflight.command ?? "-"}`
        : "尚未对最近任务执行 CAMotics 当前主机/服务器预检。",
      blocksProduction: !latestJob?.camoticsExecutionPreflight?.artifactExists
    }),
    createAcceptanceStep({
      order: 15,
...
function createV3AcceptanceRunbookShell(report) {
  const lines = [
    ...
    "RESULT_BUNDLE_ZIP=${RESULT_BUNDLE_ZIP:-${RESULT_DIR}/v3-acceptance-runbook-result-bundle.zip}",
    "cd \"$ROOT_DIR\"",
    "mkdir -p \"$RESULT_DIR\"",
    ": > \"$RESULT_STEPS_JSONL\"",
    "",
  ...
  lines.push(
    ...
    "NODE",
    "",
    "RESULT_BUNDLE_ZIP=\"$RESULT_BUNDLE_ZIP\" node - <<'NODE'",
    "const fs = require('fs');",
    "const resultPath = process.env.RESULT_JSON;",
    "const stepsPath = process.env.RESULT_STEPS_JSONL;",
    "const bundlePath = process.env.RESULT_BUNDLE_ZIP;",
    ...
    "  'camotics-cli-run-package.json'",
    "];",
    "const optionalEvidenceEntries = optionalEvidenceFiles",
    "  .filter((name) => fs.existsSync(name))",
    "  .map((name) => ({ name, content: fs.readFileSync(name) }));",
    "const files = [",
    "  { name: 'v3-acceptance-runbook-result.json', content: fs.readFileSync(resultPath) },",
    "  ...(fs.existsSync(stepsPath) ? [{ name: 'v3-acceptance-runbook-steps.jsonl', content: fs.readFileSync(stepsPath) }] : []),",
    "  ...optionalEvidenceEntries,",
    "  {",
    "    name: 'README-RUNBOOK-RESULT.md',",
    "    content: Buffer.from([",
    "      '# HeDiao3D V3 Acceptance Runbook Result Bundle',",
    "      '',",
    "      'Upload this ZIP in the HeDiao3D V3 readiness runbook-result import panel.',",
    "      '',",
    "      'Included files:',",
    "      '- v3-acceptance-runbook-result.json',",
    "      '- v3-acceptance-runbook-steps.jsonl',",
    "      ...optionalEvidenceEntries.map((entry) => '- ' + entry.name),",
    "      '',",
    ...
    "  }",
    "];",
    "fs.writeFileSync(bundlePath, createZip(files));",
    "console.log('Wrote uploadable runbook result bundle: ' + bundlePath);",
    "",
...
async function processOrchestratorJob(job, settings) {
  ...
  const camoticsExecutionPreflight = await createCamoticsExecutionPreflightReport(job, job.workDir);
  ...
  pushIfArtifactExists(job, "camotics-execution-preflight.json");
  ...
  job.result = {
    ...
    summary: {
      ...
      camoticsExecutionPreflight: camoticsExecutionPreflight ? {
        status: camoticsExecutionPreflight.status,
        canRunOnCurrentHost: camoticsExecutionPreflight.canRunOnCurrentHost,
        command: camoticsExecutionPreflight.command?.command ?? null,
        artifact: "camotics-execution-preflight.json",
        report: "camotics-execution-preflight.md",
        productionUnlockEligible: false,
        nextActions: camoticsExecutionPreflight.nextActions
      } : null,
      rotaryWrapPreviewReport,
      postprocessTraceReport,
      manufacturingSetupReport,
...
function createMachiningPackageIndex({ job, toolpath, productionGate, postprocessProfile, simulationSummary, camoticsInput, camoticsSimulationPlan, camoticsCliExecutionPlan, rotaryWrapPreviewReport, camHandoffQuality, postprocessTraceReport, camServerConfig, productionEvidenceDossier, productionClosureAudit, ncStaticAnalysis, nativeCamReadiness, camEngineSelection, openSourceCamExecutionPlan, machineControllerProfile, manufacturingSetupReport, machineAcceptanceChecklist, controllerDialectReport, machineFilePolicy, deliveryManifest }) {
  ...
  return {
    ...
    camotics: {
      ...
      executionPreflight: getFile("camotics-execution-preflight.json")?.exists ? {
        artifact: "camotics-execution-preflight.json",
        report: getFile("camotics-execution-preflight.md")?.exists ? "camotics-execution-preflight.md" : null,
        status: readJsonFileSafe(join(job.workDir, "camotics-execution-preflight.json"))?.status ?? "unknown",
        canRunOnCurrentHost: Boolean(readJsonFileSafe(join(job.workDir, "camotics-execution-preflight.json"))?.canRunOnCurrentHost),
        command: readJsonFileSafe(join(job.workDir, "camotics-execution-preflight.json"))?.command?.command ?? null
      } : null,
...
function createDeliveryManifest(job, toolpath, productionGate, repairExecution = null) {
  const files = [
    ...
    createDeliveryFile(job.id, "camotics-execution-preflight.json", "CAMotics执行预检", "report", existsSync(join(job.workDir, "camotics-execution-preflight.json")), "检查当前主机/CAM服务器是否具备运行真实 CAMotics 材料去除仿真的条件；不解锁生产。"),
    createDeliveryFile(job.id, "camotics-execution-preflight.md", "CAMotics执行预检说明", "report", existsSync(join(job.workDir, "camotics-execution-preflight.md")), "解释 CAMotics 执行预检状态、阻断原因和下一步回填动作。"),
  ...
  if (existsSync(join(job.workDir, "opencamlib-cutter-contact-report.json"))) {
    files.push(createDeliveryFile(job.id, "opencamlib-cutter-contact-report.json", "OpenCAMLib 刀具接触证据报告", "report", true, "记录 neutral-toolpath 与 OpenCAMLib 接触采样证据的哈希绑定；preview scaffold 状态下不能解锁生产。"));
  }
  for (const output of repairExecution?.outputs ?? []) {
    if (!output?.filename || !output.exists) continue;
...
function normalizeLinuxCamEvidenceUploadPlan(value) {
  ...
  return {
    ...
    endpoints: value.endpoints && typeof value.endpoints === "object"
      ...
      : null,
    files: {
      depsInstallReport: summarizeFile(files.depsInstallReport),
      preflight: summarizeFile(files.preflight),
...
async function importOrchestratorLinuxCamJobPreflight(req, jobId, res) {
  const safeJobId = decodeURIComponent(jobId);
  if (!/^[a-zA-Z0-9-]+$/.test(safeJobId)) return json(res, 400, { error: "非法 Orchestrator 任务 ID" });
  const job = orchestratorJobs.get(safeJobId) ?? readJobManifest(safeJobId);
  if (!job) return json(res, 404, { error: "找不到 Orchestrator 任务" });
  ...
  const input = await readJson(req, 5_000_000).catch((error) => ({ error }));
  if (input.error) {
    return json(res, 400, { error: input.error instanceof Error ? input.error.message : "Linux CAM 预检 JSON 无法解析" });
  }
  let preflight;
  try {
    preflight = normalizeLinuxCamJobPreflight(input.preflight ?? input, safeJobId);
  } catch (error) {
    return json(res, 400, { error: error instanceof Error ? error.message : "Linux CAM 预检格式不正确" });
  }
  await writeFile(join(workDir, "linux-cam-job-preflight.json"), JSON.stringify(preflight, null, 2), "utf8");

  const importAudit = {
    schema: "hediao3d.v3-linux-cam-job-preflight-import.v1",
    jobId: safeJobId,
    importedAt: preflight.importedAt,
    sourceName: typeof input.sourceName === "string" ? input.sourceName.slice(0, 160) : "linux-cam-job-preflight.json",
  ...
  await writeFile(join(workDir, "linux-cam-job-preflight-import.json"), JSON.stringify(importAudit, null, 2), "utf8");

  const refreshedDelivery = await refreshEvidenceDeliveryArtifacts(job);
  appendOrchestratorLog(job, `Linux CAM 整单预检已回填：${preflight.level ?? "unknown"}。`);
  job.workDir = workDir;
  job.updatedAt = preflight.importedAt;
  job.linuxCamJobPreflight = {
    importedAt: preflight.importedAt,
    level: preflight.level,
    productionUnlockEligible: false,
    artifact: "linux-cam-job-preflight.json",
    importAudit: "linux-cam-job-preflight-import.json"
  };
  job.result = job.result ?? {};
  job.result.summary = {
    ...(job.result.summary ?? {}),
    linuxCamJobPreflight: {
      level: preflight.level,
      summary: preflight.summary,
      blockerCount: preflight.blockers.length,
      warningCount: preflight.warnings.length,
  ...
  for (const filename of [
    "linux-cam-job-preflight.json",
    "linux-cam-job-preflight-import.json",
    "delivery-manifest.json",
    "operator-download-checklist.md",
    "package-integrity.json",
    "next-action-checklist.md",
  ...
  return json(res, 200, {
    ...
    productionClosureAudit: refreshedDelivery?.productionClosureAudit ?? null,
    artifacts: {
      preflight: publicArtifactUrl(safeJobId, "linux-cam-job-preflight.json"),
      importAudit: publicArtifactUrl(safeJobId, "linux-cam-job-preflight-import.json")
    }
  });
}

function normalizeLinuxCamJobPreflight(value, jobId) {
  if (!value || typeof value !== "object") throw new Error("preflight 不能为空。");
  if (value.schema !== "hediao3d.v3-linux-cam-job-preflight.v1") {
    throw new Error("preflight.schema 必须是 hediao3d.v3-linux-cam-job-preflight.v1。");
  }
  const reportJobId = typeof value.jobId === "string" ? value.jobId : jobId;
  if (reportJobId !== jobId) throw new Error("Linux CAM 预检报告与当前 job 不匹配。");
  const checks = Array.isArray(value.checks)
    ? value.checks.slice(0, 80).map((check) => ({
        id: typeof check?.id === "string" ? check.id.slice(0, 120) : null,
        type: typeof check?.type === "string" ? check.type.slice(0, 80) : null,
        command: typeof check?.command === "string" ? check.command.slice(0, 120) : null,
        moduleName: typeof check?.moduleName === "string" ? check.moduleName.slice(0, 120) : null,
        relativePath: typeof check?.relativePath === "string" ? check.relativePath.slice(0, 220) : null,
        filename: typeof check?.filename === "string" ? check.filename.slice(0, 160) : null,
        required: Boolean(check?.required),
        ok: Boolean(check?.ok),
        status: typeof check?.status === "string" ? check.status.slice(0, 120) : null,
        summary: typeof check?.summary === "string" ? check.summary.slice(0, 500) : null,
        version: typeof check?.version === "string" ? check.version.slice(0, 180) : null,
        sha256: typeof check?.sha256 === "string" ? check.sha256.slice(0, 80) : null,
        sizeBytes: Number.isFinite(Number(check?.sizeBytes)) ? Number(check.sizeBytes) : null
      }))
    : [];
  const blockers = Array.isArray(value.blockers)
    ? value.blockers.slice(0, 40).map((item) => String(item).slice(0, 240))
    : checks.filter((check) => check.required && !check.ok).map((check) => `${check.id}:${check.status}`);
  const warnings = Array.isArray(value.warnings)
    ? value.warnings.slice(0, 40).map((item) => String(item).slice(0, 240))
    : checks.filter((check) => !check.required && !check.ok).map((check) => `${check.id}:${check.status}`);
  return {
    schema: "hediao3d.v3-linux-cam-job-preflight.v1",
    jobId,
    checkedAt: typeof value.checkedAt === "string" ? value.checkedAt.slice(0, 80) : new Date().toISOString(),
    importedAt: new Date().toISOString(),
    productionUnlockEligible: false,
    level: typeof value.level === "string" ? value.level.slice(0, 80) : blockers.length ? "blocked" : "unknown",
    nativeCamServerDir: typeof value.nativeCamServerDir === "string" ? value.nativeCamServerDir.slice(0, 260) : null,
    summary: typeof value.summary === "string" ? value.summary.slice(0, 1000) : null,
...
async function createOrchestratorCamoticsExecutionPreflight(jobId, res) {
  const safeJobId = decodeURIComponent(jobId);
  if (!/^[a-zA-Z0-9-]+$/.test(safeJobId)) return json(res, 400, { error: "非法 Orchestrator 任务 ID" });
  const job = orchestratorJobs.get(safeJobId) ?? readJobManifest(safeJobId);
  if (!job) return json(res, 404, { error: "找不到 Orchestrator 任务" });
  ...
  if (!existsSync(join(workDir, "camotics-cli-run-package.json")) && existsSync(join(workDir, "camotics-cli-execution-plan.json"))) {
    await prepareCamoticsCliPackageForJob(job, workDir);
  }
  const report = await createCamoticsExecutionPreflightReport(job, workDir);
  job.workDir = workDir;
  job.updatedAt = report.createdAt;
  appendOrchestratorLog(job, `CAMotics 执行预检：${report.status}。`);
  const refreshedDelivery = await refreshEvidenceDeliveryArtifacts(job);
  job.result = job.result ?? {};
  job.result.summary = {
    ...(job.result.summary ?? {}),
    camoticsExecutionPreflight: {
      status: report.status,
      canRunOnCurrentHost: report.canRunOnCurrentHost,
      command: report.command?.command ?? null,
      artifact: "camotics-execution-preflight.json",
      report: "camotics-execution-preflight.md",
      productionUnlockEligible: false,
  ...
  await writeJobManifest(job);
  return json(res, 200, {
    ok: report.status === "ready-to-run-current-host" || report.status === "material-removal-result-imported",
    status: report.status,
    report,
    artifacts: {
      json: publicArtifactUrl(safeJobId, "camotics-execution-preflight.json"),
...
async function createCamoticsExecutionPreflightReport(job, workDir) {
  const runPackage = readJsonFileSafe(join(workDir, "camotics-cli-run-package.json"));
  const cliPackageReport = readJsonFileSafe(join(workDir, "camotics-cli-package-report.json"));
  const result = readJsonFileSafe(join(workDir, "camotics-result.json"));
  const localValidation = readJsonFileSafe(join(workDir, "camotics-result-local-validation.json"));
  const previewExists = existsSync(join(workDir, "camotics-preview.nc"));
  const templateExists = existsSync(join(workDir, "camotics-result-template.json"));
  const validatorExists = existsSync(join(workDir, "camotics-result-validate.js"));
  const operatorChecklistExists = existsSync(join(workDir, "camotics-linux-operator-checklist.md"));
  const command = detectCamEngines().find((engine) => engine.id === "camotics") ?? null;
  const resultImported = Boolean(result?.evidenceQuality?.productionEvidenceEligible && result?.synthetic === false);
  const packageReady = runPackage?.status === "ready-for-linux-camotics" && Boolean(runPackage?.preferredGcodeIdentity?.sha256);
  const requiredMissing = [
    ...(!previewExists ? ["camotics-preview.nc"] : []),
    ...(!runPackage ? ["camotics-cli-run-package.json"] : []),
    ...(!templateExists ? ["camotics-result-template.json"] : []),
    ...(!validatorExists ? ["camotics-result-validate.js"] : []),
    ...(!operatorChecklistExists ? ["camotics-linux-operator-checklist.md"] : [])
  ];
  const canRunOnCurrentHost = Boolean(packageReady && previewExists && command?.available);
  const status = resultImported
    ? "material-removal-result-imported"
    : requiredMissing.length > 0
      ? "blocked-missing-package-files"
      : !packageReady
        ? "blocked-package-not-ready"
        : !command?.available
          ? "blocked-current-host-missing-camotics"
          : "ready-to-run-current-host";
  const blockers = [
    ...requiredMissing.map((filename) => `缺少 ${filename}`),
    ...(!packageReady && requiredMissing.length === 0 ? [`CAMotics 运行包状态不是 ready-for-linux-camotics：${runPackage?.status ?? "missing"}`] : []),
    ...(!command?.available && !resultImported ? ["当前主机未检测到 camotics-cli/camotics 命令"] : [])
  ];
  const warnings = [
    "本预检不会执行材料去除仿真，也不会解锁生产 NC。",
    ...(!localValidation && result ? ["已有 camotics-result.json，但缺少 camotics-result-local-validation.json，建议补齐 Linux 本地校验报告。"] : []),
    ...(command?.available ? ["当前主机检测到 CAMotics 命令，但仍需操作者运行真实材料去除并回填结果。"] : [])
  ];
  const nextActions = resultImported
    ? ["重新生成 readiness，确认仿真层从 blocked 变为 ready/partial。"]
    : canRunOnCurrentHost
      ? [
        "在当前主机或 Linux CAM 服务器运行 camotics-linux-run.sh。",
        "按 camotics-result-template.json 填写真实 camotics-result.json。",
        "运行 node camotics-result-validate.js camotics-result.json，通过后回填结果包。"
      ]
      : [
        ...(requiredMissing.length > 0 ? ["先调用 POST /api/orchestrator/jobs/:id/camotics-cli-package 生成 CAMotics Linux 准备包。"] : []),
        ...(!command?.available ? ["把 CAMotics Linux 仿真包下载到已安装 camotics-cli/camotics 的 Linux CAM 服务器执行。"] : []),
        "回填 camotics-result-bundle.zip 或 camotics-result.json + 截图/STL + 本地校验报告。"
      ];
  const report = {
    schema: "hediao3d.camotics-execution-preflight.v1",
    jobId: job.id,
    createdAt: new Date().toISOString(),
    status,
    canRunOnCurrentHost,
    ...
    package: {
      ...
      reportStatus: cliPackageReport?.status ?? null,
      preferredGcodeSha256: runPackage?.preferredGcodeIdentity?.sha256 ?? null,
      motionLineCount: runPackage?.preferredGcodeIdentity?.motionProfile?.motionLineCount ?? null,
      machineContext: runPackage?.preferredGcodeIdentity?.machineContext ?? null
    },
    files: {
      previewExists,
      runPackageExists: Boolean(runPackage),
      templateExists,
      validatorExists,
      operatorChecklistExists,
      resultExists: Boolean(result),
      localValidationExists: Boolean(localValidation),
      screenshotExists: existsSync(join(workDir, "camotics-preview.png")),
      materialMeshExists: existsSync(join(workDir, "camotics-material-removal.stl"))
    },
    resultEvidence: result ? {
      ...
      machineContextStatus: result.evidenceQuality?.machineContext?.status ?? "missing",
      artifactEvidenceStatus: result.evidenceQuality?.status ?? "missing"
    } : null,
    blockers,
    warnings,
    nextActions,
    safetyLocks: {
      productionUnlockFromPreflight: false,
      requiresRealMaterialRemovalResult: true,
      syntheticResultAllowedForProduction: false
    }
  };
  await writeFile(join(workDir, "camotics-execution-preflight.json"), JSON.stringify(report, null, 2), "utf8");
  await writeFile(join(workDir, "camotics-execution-preflight.md"), createCamoticsExecutionPreflightMarkdown(report), "utf8");
  return report;
}

function createCamoticsExecutionPreflightMarkdown(report) {
  return [
    "# HeDiao3D CAMotics Execution Preflight",
    "",
    `Job: ${report.jobId}`,
    `Status: ${report.status}`,
    `Can run on current host: ${report.canRunOnCurrentHost ? "yes" : "no"}`,
    `Production unlock: ${report.productionUnlockEligible ? "yes" : "no"}`,
    "",
    "## Command",
    "",
    `- Available: ${report.command?.available ? "yes" : "no"}`,
    `- Command: ${report.command?.command ?? "-"}`,
    `- Version: ${report.command?.version ?? "-"}`,
    "",
    "## Package",
    "",
    `- Ready: ${report.package.ready ? "yes" : "no"}`,
    `- Status: ${report.package.status ?? "-"}`,
    `- Preview SHA-256: ${report.package.preferredGcodeSha256 ?? "-"}`,
    `- Motion lines: ${report.package.motionLineCount ?? "-"}`,
    `- Machine context: ${report.package.machineContext ? JSON.stringify(report.package.machineContext) : "-"}`,
    "",
    "## Result Evidence",
    "",
    report.resultEvidence
      ? `- eligible=${report.resultEvidence.productionEvidenceEligible} / input=${report.resultEvidence.inputIdentityStatus} / motion=${report.resultEvidence.motionConsistencyStatus} / machine=${report.resultEvidence.machineContextStatus} / artifacts=${report.resultEvidence.artifactEvidenceStatus}`
      : "- missing",
    "",
    "## Blockers",
    "",
...
function normalizeLinuxCamJobEvidenceStatus(value) {
  if (!value || typeof value !== "object") return null;
  ...
  return {
    ...
    readyForUpload: Boolean(value.readyForUpload),
    packageIntegrityOk: Boolean(value.packageIntegrityOk),
    nativeCamBundle: value.nativeCamBundle === "present" ? "present" : "missing",
    camoticsBundle: value.camoticsBundle === "present" ? "present" : "missing",
    camoticsLocalValidation: value.camoticsLocalValidation === "present" ? "present" : "missing",
    missingUploads,
...
function createOrchestratorJobSummary(job) {
  ...
  const camoticsCliPackageReport = camoticsCliPackageReportPath && existsSync(camoticsCliPackageReportPath)
    ...
    : null;
  const camoticsExecutionPreflightPath = job.workDir ? join(job.workDir, "camotics-execution-preflight.json") : null;
  const camoticsExecutionPreflight = camoticsExecutionPreflightPath && existsSync(camoticsExecutionPreflightPath)
    ? readJsonFileSafe(camoticsExecutionPreflightPath)
    : null;
  ...
  return {
    ...
    camoticsExecutionPreflight: {
      artifactExists: Boolean(camoticsExecutionPreflight),
      status: summary.camoticsExecutionPreflight?.status ?? camoticsExecutionPreflight?.status ?? null,
      canRunOnCurrentHost: Boolean(summary.camoticsExecutionPreflight?.canRunOnCurrentHost ?? camoticsExecutionPreflight?.canRunOnCurrentHost),
      command: summary.camoticsExecutionPreflight?.command ?? camoticsExecutionPreflight?.command?.command ?? null,
      artifact: camoticsExecutionPreflight ? "camotics-execution-preflight.json" : null,
      report: job.workDir && existsSync(join(job.workDir, "camotics-execution-preflight.md")) ? "camotics-execution-preflight.md" : null,
      nextActions: summary.camoticsExecutionPreflight?.nextActions ?? camoticsExecutionPreflight?.nextActions ?? []
    }
  };
}
...
async function getOrchestratorLinuxCamJobPackage(jobId, res) {
  ...
  files.push({
    name: `${root}/preflight-linux-cam-job.mjs`,
  ...
  files.push({
    name: `${root}/upload-linux-cam-evidence.mjs`,
    content: createLinuxCamJobEvidenceUploadScript(manifest)
  });
...
function createLinuxCamJobPackageManifest({ jobId, runPackage, deliveryManifest, packageIntegrity, openCamLibManifest, openCamLibFiles, camoticsFiles, references }) {
  return {
    ...
    requiredSequence: [
      "解压本包到 Linux CAM 服务器。",
      "可选：设置 HEDIAO3D_NATIVE_CAM_SERVER_DIR=/path/to/hediao3d-native-cam-server。",
...
function createLinuxCamJobPackageReadme(manifest) {
  ...
  return [
    ...
    "HEDIAO3D_V3_API_BASE=http://127.0.0.1:8787 node upload-linux-cam-evidence.mjs .",
    "```",
    "",
    ...
    "",
    "## 一键回填到 V3 Orchestrator",
    "",
    "完成 Linux CAM 与 CAMotics 后，先本地校验，再上传证据包：",
    "",
    "```bash",
    "node validate-linux-cam-job.mjs .",
    "HEDIAO3D_V3_API_BASE=http://你的V3服务器:8787 node upload-linux-cam-evidence.mjs .",
    "```",
    "",
...
function createLinuxCamJobRunScript(manifest) {
  return [
    "#!/usr/bin/env bash",
    "set -u",
    "",
    "ROOT=\"$(cd \"$(dirname \"${BASH_SOURCE[0]}\")\" && pwd)\"",
    "REPORT=\"$ROOT/linux-cam-job-run-report.json\"",
    "NATIVE_DIR=\"${HEDIAO3D_NATIVE_CAM_SERVER_DIR:-}\"",
    "if [[ -z \"$NATIVE_DIR\" && -d \"$ROOT/../hediao3d-native-cam-server\" ]]; then NATIVE_DIR=\"$ROOT/../hediao3d-native-cam-server\"; fi",
    "if [[ -z \"$NATIVE_DIR\" && -d \"$ROOT/hediao3d-native-cam-server\" ]]; then NATIVE_DIR=\"$ROOT/hediao3d-native-cam-server\"; fi",
    "",
    "log() { printf '[HeDiao3D Linux CAM] %s\\n' \"$*\"; }",
    "append_json_note() { :; }",
    "",
    `log "Job ${manifest.jobId}: Linux CAM package execution started."`,
    "log \"Package root: $ROOT\"",
    "log \"Native CAM server dir: ${NATIVE_DIR:-missing}\"",
    "if [[ -f \"$ROOT/preflight-linux-cam-job.mjs\" ]]; then",
    "  log \"Running Linux CAM job preflight.\"",
    "  node \"$ROOT/preflight-linux-cam-job.mjs\" \"$ROOT\" || true",
    "fi",
    "",
    "cat > \"$REPORT\" <<'JSON'",
    JSON.stringify({
      schema: "hediao3d.v3-linux-cam-job-run-report.v1",
      jobId: manifest.jobId,
      startedAt: "set-by-run-script",
      productionUnlockEligible: false,
      note: "This report is initialized by run-linux-cam-job.sh; validate-linux-cam-job.mjs writes the authoritative local validation."
    }, null, 2),
    "JSON",
    "",
    "if [[ -z \"$NATIVE_DIR\" || ! -d \"$NATIVE_DIR\" ]]; then",
    "  log \"Native CAM server package not found. Set HEDIAO3D_NATIVE_CAM_SERVER_DIR and rerun.\"",
    "else",
    "  log \"Copying OpenCAMLib candidate inputs into Native CAM server directory.\"",
    "  cp -f \"$ROOT/native-cam/opencamlib-candidate-inputs/\"* \"$NATIVE_DIR\"/",
    "  (",
    "    cd \"$NATIVE_DIR\" || exit 0",
    "    if [[ -f native-cam-server-package-self-check.mjs ]]; then node native-cam-server-package-self-check.mjs . || true; fi",
    "    if [[ -f opencamlib-real-candidate-run.mjs ]]; then node opencamlib-real-candidate-run.mjs . || true; fi",
    "    if [[ -f native-cam-real-output-check.sh ]]; then bash native-cam-real-output-check.sh . || true; fi",
    "  )",
    "  if [[ -f \"$NATIVE_DIR/native-cam-real-output-bundle.zip\" ]]; then",
    "    cp -f \"$NATIVE_DIR/native-cam-real-output-bundle.zip\" \"$ROOT/native-cam-real-output-bundle.zip\"",
    "    log \"Copied native-cam-real-output-bundle.zip back to package root.\"",
    "  fi",
    "fi",
    "",
    "CAMOTICS_WORK=\"$ROOT/camotics-work\"",
    "mkdir -p \"$CAMOTICS_WORK\"",
    ...
    "fi",
    "",
    "if [[ -f \"$CAMOTICS_WORK/camotics-result.json\" && -f \"$CAMOTICS_WORK/run/camotics-result-validate.js\" ]]; then",
    "  log \"Validating CAMotics result.\"",
    "  (cd \"$CAMOTICS_WORK\" && node run/camotics-result-validate.js camotics-result.json || true)",
    "else",
    "  log \"camotics-result.json not found yet; run CAMotics/equivalent simulation and rerun validator.\"",
    "fi",
    "if [[ -f \"$CAMOTICS_WORK/camotics-result-bundle.zip\" ]]; then cp -f \"$CAMOTICS_WORK/camotics-result-bundle.zip\" \"$ROOT/camotics-result-bundle.zip\"; fi",
    "",
    "if [[ -f \"$ROOT/validate-linux-cam-job.mjs\" ]]; then",
    "  node \"$ROOT/validate-linux-cam-job.mjs\" \"$ROOT\" || true",
    "fi",
    "log \"Linux CAM package execution finished. Review linux-cam-job-local-validation.json.\"",
    ""
  ].join("\n");
}

function createLinuxCamJobPreflightScript(manifest) {
  return [
    "#!/usr/bin/env node",
    ...
    "const root = resolve(process.argv[2] ?? scriptDir);",
    "const nativeDir = process.env.HEDIAO3D_NATIVE_CAM_SERVER_DIR ? resolve(process.env.HEDIAO3D_NATIVE_CAM_SERVER_DIR) : null;",
    ...
    "}",
    "",
    "function fileCheck(id, relativePath, required = true) {",
    "  const fullPath = join(root, relativePath);",
    "  if (!existsSync(fullPath)) return { id, type: 'file', relativePath, required, status: required ? 'missing' : 'optional-missing', ok: !required };",
    "  return { id, type: 'file', relativePath, required, status: 'present', ok: true, sizeBytes: statSync(fullPath).size, sha256: hashFile(relativePath) };",
    "}",
    "",
    "function commandCheck(id, command, args = ['--version'], required = true) {",
    "  const lookup = process.platform === 'win32' ? spawnSync('where', [command], { encoding: 'utf8' }) : spawnSync('sh', ['-lc', `command -v ${command}`], { encoding: 'utf8' });",
    "  const available = lookup.status === 0;",
    "  if (!available) return { id, type: 'command', command, required, status: required ? 'missing' : 'optional-missing', ok: !required, summary: `${command} not found in PATH.` };",
    "  const version = spawnSync(command, args, { encoding: 'utf8', timeout: 8000 });",
    "  const output = `${version.stdout ?? ''}${version.stderr ?? ''}`.trim().slice(0, 500);",
    "  return { id, type: 'command', command, required, status: 'available', ok: true, path: lookup.stdout.trim().split(/\\r?\\n/)[0] ?? null, version: output || null };",
    "}",
    "",
    "function pythonModuleCheck(id, moduleName, required = true) {",
    "  const python = ['python3', 'python'].find((name) => commandCheck(`${id}-${name}-lookup`, name, ['--version'], false).ok);",
    "  if (!python) return { id, type: 'python-module', moduleName, required, status: required ? 'missing-python' : 'optional-missing-python', ok: !required };",
    "  const probe = spawnSync(python, ['-c', `import ${moduleName}; print(getattr(${moduleName}, '__version__', 'available'))`], { encoding: 'utf8', timeout: 10000 });",
    "  if (probe.status !== 0) return { id, type: 'python-module', moduleName, python, required, status: required ? 'missing-module' : 'optional-missing-module', ok: !required, summary: `${moduleName} import failed`, detail: `${probe.stderr ?? probe.stdout ?? ''}`.trim().slice(0, 500) };",
    "  return { id, type: 'python-module', moduleName, python, required, status: 'available', ok: true, version: `${probe.stdout ?? ''}`.trim().slice(0, 120) || 'available' };",
    "}",
    "",
    "function nativeScriptCheck(filename) {",
    "  if (!nativeDir) return { id: `native:${filename}`, type: 'native-cam-script', filename, required: true, status: 'missing-native-dir', ok: false, summary: 'Set HEDIAO3D_NATIVE_CAM_SERVER_DIR to the extracted Native CAM server package.' };",
    "  const fullPath = join(nativeDir, filename);",
    ...
    "  commandCheck('python-runtime', 'python', ['--version'], false),",
    "  commandCheck('camotics-cli', 'camotics', ['--version'], true),",
    "  commandCheck('freecadcmd-cli', 'freecadcmd', ['--version'], false),",
    "  commandCheck('blender-cli', 'blender', ['--version'], false),",
    "  pythonModuleCheck('opencamlib-python', 'ocl', true),",
    "  fileCheck('manifest', 'linux-cam-job-package-manifest.json', true),",
    "  fileCheck('opencamlib-job', 'native-cam/opencamlib-candidate-inputs/job.json', true),",
    "  fileCheck('opencamlib-plan', 'native-cam/opencamlib-candidate-inputs/opencamlib-kernel-plan.json', true),",
    "  fileCheck('opencamlib-model', 'native-cam/opencamlib-candidate-inputs/repaired-model.stl', true),",
    "  fileCheck('camotics-preview', 'camotics/inputs/camotics-preview.nc', true),",
    "  fileCheck('camotics-run-package', 'camotics/run/camotics-cli-run-package.json', true),",
    "  nativeScriptCheck('native-cam-server-package-self-check.mjs'),",
    "  nativeScriptCheck('opencamlib-real-candidate-run.mjs'),",
    "  nativeScriptCheck('native-cam-real-output-check.sh')",
    "];",
    "",
    ...
    "  checkedAt: new Date().toISOString(),",
    "  productionUnlockEligible: false,",
    "  level: blockers.length ? 'blocked' : warnings.length ? 'ready-with-warnings' : 'ready',",
    "  nativeCamServerDir: nativeDir,",
    "  summary: blockers.length",
    "    ? `Linux CAM preflight blocked by ${blockers.length} required check(s).`",
    "    : warnings.length",
    "      ? `Linux CAM preflight ready with ${warnings.length} optional warning(s).`",
    "      : 'Linux CAM preflight ready: required OpenCAMLib, CAMotics, inputs, and Native CAM scripts are available.',",
    ...
    "    ? ['Install missing required tools/modules, set HEDIAO3D_NATIVE_CAM_SERVER_DIR, then rerun node preflight-linux-cam-job.mjs .', 'Do not upload production evidence until preflight is ready and real CAM/CAMotics bundles are generated.']",
    "    : ['Run bash run-linux-cam-job.sh, then node validate-linux-cam-job.mjs .', 'Upload evidence only after native-cam-real-output-bundle.zip and camotics-result-bundle.zip exist.']",
    "};",
    "",
...
function createLinuxCamJobDependencyInstallScript(manifest) {
  return [
    ...
    "ROOT=\"$(cd \"$(dirname \"${BASH_SOURCE[0]}\")\" && pwd)\"",
    "REPORT=\"$ROOT/linux-cam-deps-install-report.json\"",
    "SUDO=\"\"",
    ...
    "json_array() { local first=1; printf '['; for item in \"$@\"; do if [[ \"$first\" == \"0\" ]]; then printf ','; fi; first=0; json_escape \"$item\"; done; printf ']'; }",
    "run() {",
    "  record_command \"$*\"",
    "  if [[ \"$EXECUTE\" == \"1\" ]]; then",
    "    log \"+ $*\"",
    "    if \"$@\"; then RESULTS+=(\"ok:$*\"); else RESULTS+=(\"failed:$*\"); return 1; fi",
    "  else",
    "    log \"dry-run: $*\"",
    "    RESULTS+=(\"dry-run:$*\")",
    "  fi",
    ...
    "  if [[ \"$EXECUTE\" == \"1\" ]]; then status=\"executed\"; fi",
    "  if printf '%s\\n' \"${RESULTS[@]}\" | grep -q '^failed:'; then status=\"failed\"; fi",
    "  {",
    "    printf '{\\n'",
    "    printf '  \"schema\": \"hediao3d.v3-linux-cam-deps-install-report.v1\",\\n'",
    "    printf '  \"jobId\": '; json_escape \"$JOB_ID\"; printf ',\\n'",
    "    printf '  \"createdAt\": '; json_escape \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\"; printf ',\\n'",
    "    printf '  \"mode\": '; json_escape \"$EXECUTE\"; printf ',\\n'",
    "    printf '  \"status\": '; json_escape \"$status\"; printf ',\\n'",
    "    printf '  \"productionUnlockEligible\": false,\\n'",
    "    printf '  \"commands\": '; json_array \"${COMMANDS[@]}\"; printf ',\\n'",
    "    printf '  \"results\": '; json_array \"${RESULTS[@]}\"; printf ',\\n'",
    "    printf '  \"summary\": '; json_escape \"Dependency helper finished with status $status. Rerun preflight before CAM execution.\"; printf '\\n'",
    "    printf '}\\n'",
    "  } > \"$REPORT\"",
    "  log \"Wrote $REPORT\"",
    "}",
...
function createLinuxCamJobEvidenceUploadScript(manifest) {
  return [
    "#!/usr/bin/env node",
    "import { existsSync, readFileSync, writeFileSync, statSync } from 'node:fs';",
    "import { createHash } from 'node:crypto';",
    ...
    "const root = resolve(rootArg ?? scriptDir);",
    "const apiBase = String(process.env.HEDIAO3D_V3_API_BASE ?? process.env.V3_API_BASE ?? '').replace(/\\/$/, '');",
    "const retryCommand = 'HEDIAO3D_V3_API_BASE=http://127.0.0.1:8787 node upload-linux-cam-evidence.mjs .';",
    `const manifest = ${JSON.stringify(manifest, null, 2)};`,
    "",
    "function fileInfo(relativePath, required = true) {",
    "  const fullPath = join(root, relativePath);",
    "  if (!existsSync(fullPath)) return { relativePath, required, exists: false, status: required ? 'missing' : 'optional-missing' };",
    ...
    "  if (!response.ok) {",
    "    const error = data?.error ?? `${response.status} ${path}`;",
    ...
    "}",
    "",
    "const preflight = fileInfo('linux-cam-job-preflight.json', false);",
    "const depsInstallReport = fileInfo('linux-cam-deps-install-report.json', false);",
    ...
    "  },",
    "  files: { depsInstallReport, preflight, validation, nativeBundle, camoticsBundle },",
    "  readyForUpload: validation.exists && nativeBundle.exists && camoticsBundle.exists,",
    "  summary: validation.exists && nativeBundle.exists && camoticsBundle.exists",
    "    ? 'Linux CAM validation and both evidence bundles are ready to upload through Orchestrator.'",
    "    : 'One or more Linux CAM upload files are missing; run CAM/CAMotics and validate again.'",
    "};",
    "",
    "const report = {",
    "  schema: 'hediao3d.v3-linux-cam-evidence-upload-report.v1',",
    ...
    "  retryCommand,",
    "  resumeSafe: true,",
    "  nextActions: uploadPlan.readyForUpload",
    "    ? (dryRun ? ['Set HEDIAO3D_V3_API_BASE or V3_API_BASE to the Orchestrator URL.', 'Run the uploader without --dry-run from this package root.'] : ['If upload fails, rerun the same command; successful imports are safe to re-submit for audit review.', 'Regenerate V3 readiness after upload completes.'])",
    "    : ['Run bash run-linux-cam-job.sh on the Linux CAM server.', 'Run node validate-linux-cam-job.mjs . after Native CAM and CAMotics bundles are present.', 'Rerun this uploader after linux-cam-job-local-validation.json, native-cam-real-output-bundle.zip and camotics-result-bundle.zip exist.'],",
    "  ok: false,",
    "  summary: uploadPlan.summary",
    "};",
    "",
    "let currentUploadId = null;",
    "",
    "function writeReport() {",
    "  report.completedCount = report.uploads.filter((item) => item.status === 'uploaded').length;",
    "  writeFileSync(join(root, 'linux-cam-evidence-upload-report.json'), JSON.stringify(report, null, 2));",
    "}",
    "",
    "async function uploadStep(id, endpoint, body, details, run) {",
    "  currentUploadId = id;",
    "  report.phase = `uploading:${id}`;",
    "  writeReport();",
    "  const result = await run();",
    "  report.uploads.push({ id, status: 'uploaded', endpoint, ...details(result) });",
    "  report.phase = `uploaded:${id}`;",
    ...
    "",
    "async function main() {",
    "  if (!uploadPlan.readyForUpload) {",
    "    report.phase = 'missing-upload-files';",
    "    report.summary = uploadPlan.summary;",
    "    writeReport();",
    "    console.log(JSON.stringify(report, null, 2));",
    "    process.exitCode = 2;",
    "    return;",
    ...
    "      { id: 'camotics-result', status: 'planned', endpoint: uploadPlan.endpoints.evidenceBundle, sourceName: 'camotics-result-bundle.zip' }",
    "    ];",
    "    report.completedCount = 0;",
    "    report.nextActions = ['Set HEDIAO3D_V3_API_BASE or V3_API_BASE to the Orchestrator URL.', 'Run: ' + retryCommand, 'After upload, regenerate V3 readiness and keep production NC locked until field evidence is complete.'];",
    "    writeReport();",
    "    console.log(JSON.stringify(report, null, 2));",
    "    return;",
    "  }",
    "  report.phase = 'upload-started';",
    "  report.nextActions = ['Upload in progress. If interrupted, rerun: ' + retryCommand, 'After completion, regenerate V3 readiness and continue air-run/trial evidence.'];",
    "  writeReport();",
    ...
    "    await uploadStep('linux-cam-deps-install-report', uploadPlan.endpoints.depsInstallReport, null, (depsInstallResult) => ({ level: depsInstallResult?.report?.status ?? null }), () => postJson(uploadPlan.endpoints.depsInstallReport, { report: depsInstallJson, sourceName: 'linux-cam-deps-install-report.json' }));",
    "  }",
    "  if (preflight.exists) {",
    "    const preflightJson = JSON.parse(readFileSync(join(root, 'linux-cam-job-preflight.json'), 'utf8'));",
    "    await uploadStep('linux-cam-job-preflight', uploadPlan.endpoints.preflight, null, (preflightResult) => ({ level: preflightResult?.preflight?.level ?? null }), () => postJson(uploadPlan.endpoints.preflight, { preflight: preflightJson, sourceName: 'linux-cam-job-preflight.json' }));",
    "  }",
    "  const validationJson = JSON.parse(readFileSync(join(root, 'linux-cam-job-local-validation.json'), 'utf8'));",
    "  await uploadStep('linux-cam-job-validation', uploadPlan.endpoints.validation, null, (validationResult) => ({ level: validationResult?.validation?.level ?? null }), () => postJson(uploadPlan.endpoints.validation, { validation: validationJson, sourceName: 'linux-cam-job-local-validation.json' }));",
    "  await uploadStep('native-cam-real-output', uploadPlan.endpoints.evidenceBundle, null, (nativeResult) => ({ level: nativeResult?.level ?? null, importId: nativeResult?.id ?? null }), () => postJson(uploadPlan.endpoints.evidenceBundle, { bundleDataUrl: dataUrlFor('native-cam-real-output-bundle.zip'), sourceName: 'native-cam-real-output-bundle.zip' }));",
    "  await uploadStep('camotics-result', uploadPlan.endpoints.evidenceBundle, null, (camoticsResult) => ({ level: camoticsResult?.simulationEvidence?.level ?? null, productionUnlockEligible: Boolean(camoticsResult?.simulationEvidence?.productionUnlockEligible) }), () => postJson(uploadPlan.endpoints.evidenceBundle, { bundleDataUrl: dataUrlFor('camotics-result-bundle.zip'), sourceName: 'camotics-result-bundle.zip' }));",
    "  report.ok = true;",
    "  report.phase = 'evidence-uploaded-report-pending';",
    "  report.summary = 'Linux CAM evidence uploaded to HeDiao3D V3. Regenerate readiness and continue air-run/trial evidence.';",
    "  report.nextActions = ['Upload completed. Regenerate V3 readiness.', 'Continue air-run, soft-material trial and machine acceptance before production NC is unlocked.'];",
    "  writeReport();",
    "  await uploadStep('linux-cam-evidence-upload-report', uploadPlan.endpoints.uploadReport, null, (uploadReportResult) => ({ level: uploadReportResult?.report?.ok ? 'ok' : 'review' }), () => postJson(uploadPlan.endpoints.uploadReport, { report, sourceName: 'linux-cam-evidence-upload-report.json' }));",
    "  report.phase = 'uploaded-complete';",
    "  writeReport();",
    "  console.log(JSON.stringify(report, null, 2));",
    "}",
    "",
    "main().catch((error) => {",
    "  report.ok = false;",
    "  report.phase = report.uploads.length > 0 ? 'partial-upload-failed' : 'upload-failed-before-first-request';",
    "  report.failedUpload = currentUploadId ? { id: currentUploadId, phase: report.phase } : null;",
    "  report.summary = error instanceof Error ? error.message : String(error);",
    "  report.error = { message: report.summary, status: error?.status ?? null, response: error?.response ?? null };",
    "  report.nextActions = ['Fix the reported error, then rerun: ' + retryCommand, 'It is safe to rerun the uploader; already imported reports/bundles remain audit evidence.', 'Do not treat this upload report as production unlock evidence.'];",
    "  writeReport();",
...
function createLinuxCamJobLocalValidatorScript(manifest) {
  return [
    "#!/usr/bin/env node",
    "import { existsSync, readFileSync, writeFileSync, statSync } from 'node:fs';",
    "import { createHash } from 'node:crypto';",
    ...
    `const manifest = ${JSON.stringify(manifest, null, 2)};`,
    "",
    "function fileSummary(relativePath, required = true) {",
    "  const fullPath = join(root, relativePath);",
    "  if (!existsSync(fullPath)) return { relativePath, required, exists: false, status: required ? 'missing' : 'optional-missing' };",
    "  const bytes = readFileSync(fullPath);",
    "  return { relativePath, required, exists: true, status: 'present', sizeBytes: statSync(fullPath).size, sha256: createHash('sha256').update(bytes).digest('hex') };",
    ...
    "  fileSummary('native-cam/opencamlib-candidate-inputs/opencamlib-kernel-plan.json'),",
    "  fileSummary('native-cam/opencamlib-candidate-inputs/repaired-model.stl'),",
    "  fileSummary('camotics/inputs/camotics-preview.nc'),",
    "  fileSummary('camotics/run/camotics-cli-run-package.json'),",
    "  fileSummary('camotics/run/camotics-result-template.json'),",
    ...
    "  try { depsInstallReport = JSON.parse(readFileSync(join(root, 'linux-cam-deps-install-report.json'), 'utf8')); } catch { depsInstallReport = { status: 'invalid-json', summary: 'linux-cam-deps-install-report.json is not valid JSON.' }; }",
    "}",
    "const preflightFile = checks.find((check) => check.relativePath === 'linux-cam-job-preflight.json');",
    "let preflight = null;",
    "if (preflightFile?.exists) {",
    ...
    "  schema: 'hediao3d.v3-linux-cam-job-evidence-status.v1',",
    "  phase: missingRequired.length ? 'invalid-package' : hashMismatches.length ? 'package-hash-mismatch' : productionEvidenceReady ? 'ready-for-upload' : 'waiting-for-linux-evidence',",
    ...
    "  missingUploads,",
    "  requiredFileCount: requiredChecks.length,",
    "  presentRequiredFileCount: requiredChecks.filter((check) => check.exists).length,",
    ...
    "    summary: nativeBundle?.exists ? 'Ready to upload through the unified Linux CAM evidence endpoint.' : 'Run native-cam-real-output-check.sh to create this bundle.'",
    "  },",
    "  {",
    "    id: 'camotics-material-removal',",
    "    label: 'CAMotics材料去除包',",
    ...
    "  },",
    "  checks,",
    "  nextActions: productionEvidenceReady",
    "    ? ['Upload native-cam-real-output-bundle.zip.', 'Upload camotics-result-bundle.zip.', 'Regenerate V3 readiness and continue air-run/trial evidence.']",
    "    : ['Run bash run-linux-cam-job.sh after setting HEDIAO3D_NATIVE_CAM_SERVER_DIR.', 'Run real CAMotics/equivalent simulation and produce camotics-result-bundle.zip.', 'Rerun node validate-linux-cam-job.mjs .']",
    "};",
    "",
