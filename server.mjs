...
function createLinuxCamJobEvidenceUploadScript(manifest) {
  return [
    ...
    "    const preflightResult = await postJson(uploadPlan.endpoints.preflight, { preflight: preflightJson, sourceName: 'linux-cam-job-preflight.json' });",
    "    report.uploads.push({ id: 'linux-cam-job-preflight', status: 'uploaded', endpoint: uploadPlan.endpoints.preflight, level: preflightResult?.preflight?.level ?? null });",
    "  }",
    "  const validationJson = JSON.parse(readFileSync(join(root, 'linux-cam-job-local-validation.json'), 'utf8'));",
    "  const validationResult = await postJson(uploadPlan.endpoints.validation, { validation: validationJson, sourceName: 'linux-cam-job-local-validation.json' });",
    "  report.uploads.push({ id: 'linux-cam-job-validation', status: 'uploaded', endpoint: uploadPlan.endpoints.validation, level: validationResult?.validation?.level ?? null });",
    "  const nativeResult = await postJson(uploadPlan.endpoints.evidenceBundle, { bundleDataUrl: dataUrlFor('native-cam-real-output-bundle.zip'), sourceName: 'native-cam-real-output-bundle.zip' });",
    "  report.uploads.push({ id: 'native-cam-real-output', status: 'uploaded', endpoint: uploadPlan.endpoints.evidenceBundle, level: nativeResult?.level ?? null, importId: nativeResult?.id ?? null });",
    "  const camoticsResult = await postJson(uploadPlan.endpoints.evidenceBundle, { bundleDataUrl: dataUrlFor('camotics-result-bundle.zip'), sourceName: 'camotics-result-bundle.zip' });",
