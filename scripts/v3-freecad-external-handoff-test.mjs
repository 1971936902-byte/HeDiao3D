...
const port = Number(process.env.V3_FREECAD_HANDOFF_PORT ?? 8794);
const baseUrl = `http://127.0.0.1:${port}`;
const importedModelName = `v3-freecad-external-${Date.now()}.stl`;
...
const fixtureDir = mkdtempSync(join(tmpdir(), "hediao3d-freecad-external-handoff-"));
const camoticsFixturePath = join(fixtureDir, "camotics-freecad-result-fixture.json");
const runnerPath = resolve("adapters", "freecad", "freecad_runner.py");
...
async function main() {
  ...
  server = spawn(process.execPath, ["server.mjs"], {
    ...
    env: {
      ...
      HEDIAO3D_FREECAD_EXTERNAL_COMMAND_JSON: JSON.stringify([process.env.PYTHON ?? "python", runnerPath]),
  ...
  assert(toolpath.includes("HeDiao3D FreeCAD external runner fixture"), "toolpath should come from FreeCAD external runner");
...
function writeCamoticsFixture(filePath) {
  writeFileSync(filePath, JSON.stringify({
    ...
    status: "completed",
    riskLevel: "ready",
    summary: "Imported non-synthetic CAMotics fixture for FreeCAD external G-code handoff.",
    metrics: {
      motionLineCount: 12,
      zMin: -1.2,
      zMax: 22,
...
function writeClosedReliefStl(filePath) {
  ...
  writeFileSync(filePath, `solid freecad_closed_relief\n${facets.join("")}endsolid freecad_closed_relief\n`);
}
