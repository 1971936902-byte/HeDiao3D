...
- `freecad/freecad_cam_job.py`: scriptable FreeCAD Path Workbench adapter skeleton. It validates the protocol, detects FreeCAD/Path Python modules, writes `freecad-cam-plan.json` with per-operation `freecadOperationHint`/`validationState`, and writes a reviewable `freecad-run-template.py` containing the deployment-side model import, Path Job, ToolController, operation-mapping guard and postprocessor handoff skeleton. Production G-code remains locked unless the deployment server sets `HEDIAO3D_FREECAD_EXPERIMENTAL_OUTPUT=true` and the Path operation recipe has been validated.
- For deployment, `freecad/freecad_cam_job.py` can also call an external FreeCAD command with `HEDIAO3D_FREECAD_EXTERNAL_COMMAND_JSON='["FreeCADCmd","/opt/hediao/freecad_runner.py"]'` or a Python wrapper command. The command receives `job.json`, `freecad-cam-plan.json` and the target `toolpath.nc` path as arguments. Its output must be non-empty G-code with G0/G1 motion; HeDiao3D then ingests it into the shared preview, CAMotics and safety-gate pipeline. To classify the output as `production-candidate`, the command must also write `toolpath.nc.cam-proof.json` or the path named by `HEDIAO3D_FREECAD_CAM_OUTPUT_PROOF_JSON`, using schema `hediao3d.freecad-cam-output-report.v1` or `hediao3d.external-cam-output-report.v1`, with matching `gcodeSha256`, `quality.productionCandidate=true` and `quality.postprocessEligible=true`. Otherwise the handoff is classified as `missing-cam-proof` or `cam-proof-review`.
- `freecad/freecad_runner.py` is the deployable external-command scaffold. It validates job/plan inputs and fails closed unless `HEDIAO3D_FREECAD_RUNNER_FIXTURE_OUTPUT=true` is set for contract testing. The generated `freecad-run-template.py` is the reviewed starting point for replacing fixture mode with real FreeCAD Path Job/ToolController/operation/postprocessor output before production unlock.
...
- For deployment, `blendercam/blendercam_job.py` can also call an external Blender/FabexCNC command with `HEDIAO3D_BLENDERCAM_EXTERNAL_COMMAND_JSON='["blender","--background","--python","/opt/hediao/blendercam_runner.py","--"]'` or a Python wrapper command. The command receives `job.json`, `blendercam-cam-plan.json` and the target `toolpath.nc` path as arguments. Its output must be non-empty G-code with G0/G1 motion; HeDiao3D then ingests it into the shared preview, CAMotics and safety-gate pipeline. To classify the output as `production-candidate`, the command must also write `toolpath.nc.cam-proof.json` or the path named by `HEDIAO3D_BLENDERCAM_CAM_OUTPUT_PROOF_JSON`, using schema `hediao3d.blendercam-cam-output-report.v1` or `hediao3d.external-cam-output-report.v1`, with matching `gcodeSha256`, `quality.productionCandidate=true` and `quality.postprocessEligible=true`. Otherwise the handoff is classified as `missing-cam-proof` or `cam-proof-review`.
- `blendercam/blendercam_runner.py` is the deployable external-command scaffold. It validates job/plan inputs and fails closed unless `HEDIAO3D_BLENDERCAM_RUNNER_FIXTURE_OUTPUT=true` is set for contract testing. Real BlenderCAM/FabexCNC operation setup and postprocessor output must replace fixture mode before production unlock.
...
- `camotics/camotics_cli_prepare.js`: deployment-side preparation helper for Linux CAM servers. It reads an Orchestrator job folder, validates `camotics-cli-execution-plan.json`, hashes `camotics-preview.nc`, records the preview motion profile and machine context (`ROTARY_WRAP_AXIS` / `ROTARY_WRAP_PER_REV_MM` / `LENGTH_AXIS`), and writes `camotics-cli-run-package.json`, `camotics-result-template.json` and `camotics-linux-run.sh`. The result template also records the SHA-256 of the current `camotics-cli-run-package.json`; if that run package exists, the CAMotics import adapter requires `inputs.camoticsCliRunPackageSha256` and `inputs.machineContext` to match before the material-removal result can become production evidence. This helper only prepares a real CAMotics run package; it never unlocks production NC.
- `opencamlib/opencamlib_job.py`: OpenCAMLib geometry-kernel adapter skeleton. It validates the protocol, detects `opencamlib`/`ocl`, writes `opencamlib-kernel-plan.json`, and writes a fail-closed `opencamlib-run-template.py` with the strict HeDiao3D neutral output writer, `hediao3d.opencamlib-cutter-contact-report.v1` schema, hash binding and `productionCandidateCriteria`. It supports importing real non-synthetic `hediao3d.neutral-toolpath.v1` via `HEDIAO3D_OPENCAMLIB_NEUTRAL_JSON=/path/to/neutral-toolpath.json`, can run the bundled STL heightfield preview runner with `HEDIAO3D_OPENCAMLIB_EXPERIMENTAL_OUTPUT=true` plus `HEDIAO3D_OPENCAMLIB_HEIGHTFIELD_PREVIEW=true`, and also supports a gated synthetic handoff for contract tests via `HEDIAO3D_OPENCAMLIB_SYNTHETIC_NEUTRAL_OUTPUT=true`; synthetic output validates Orchestrator ingestion only and is not real CAM output.
...
- `opencamlib/opencamlib_runner.py` is the deployable external-command scaffold. It validates job/plan inputs, writes an ASCII/binary STL geometry summary when possible, and fails closed unless `HEDIAO3D_OPENCAMLIB_RUNNER_FIXTURE_OUTPUT=true` or the experimental `HEDIAO3D_OPENCAMLIB_RUNNER_HEIGHTFIELD_OUTPUT=true` preview sampler is enabled. The adapter-level `HEDIAO3D_OPENCAMLIB_HEIGHTFIELD_PREVIEW=true` switch automatically calls this bundled runner and marks the result as `heightfieldPreview`/`previewScaffold`; it also writes `opencamlib-cutter-envelope-report.json` with cutter-radius envelope, hit/fallback rates and depth statistics for audit. Real OpenCAMLib drop-cutter output must replace fixture/heightfield preview modes before production unlock; GLB/GLTF inputs still need upstream STL/OBJ conversion before this runner can perform geometry-kernel work.
- OpenCAMLib `handoffEvidence.productionCandidate=true` now requires a non-preview `hediao3d.opencamlib-cutter-contact-report.v1` with `quality.productionCandidate=true`, `quality.postprocessEligible=true`, `quality.previewScaffold=false`, and matching model/plan/neutral SHA-256 identity. Neutral points without that report are classified as `missing-contact-report`, `preview-scaffold`, or `contact-report-review`.
- FreeCAD/BlenderCAM `handoffEvidence.productionCandidate=true` now requires the companion CAM output proof described above. Non-fixture G-code without proof is still useful for air-run and pipeline testing, but it is not a production CAM candidate.
