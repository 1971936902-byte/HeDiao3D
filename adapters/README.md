...
- `opencamlib/opencamlib_runner.py` is the deployable external-command scaffold. It validates job/plan inputs, writes an ASCII STL geometry summary when possible, and fails closed unless `HEDIAO3D_OPENCAMLIB_RUNNER_FIXTURE_OUTPUT=true` is set for contract testing. Real OpenCAMLib drop-cutter output must replace fixture mode before production unlock; GLB/GLTF inputs still need upstream STL/OBJ conversion before this runner can perform geometry-kernel work.
