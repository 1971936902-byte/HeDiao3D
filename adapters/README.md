...
- OpenCAMLib `handoffEvidence.productionCandidate=true` now requires a non-preview `hediao3d.opencamlib-cutter-contact-report.v1` with `quality.productionCandidate=true` and `quality.postprocessEligible=true`; neutral points without that report are classified as `missing-contact-report` or `contact-report-review`.
