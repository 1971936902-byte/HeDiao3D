...
def load_cam_output_proof(gcode_path: Path) -> Optional[Dict[str, Any]]:
    proof_override = os.environ.get("HEDIAO3D_BLENDERCAM_CAM_OUTPUT_PROOF_JSON")
    ...
    for candidate in candidates:
        ...
        try:
            ...
            return {
                "schema": "hediao3d.blendercam-cam-output-report.v1",
                "path": str(candidate),
                "valid": False,
...
def evaluate_cam_output_proof(proof: Optional[Dict[str, Any]], gcode: str, gcode_path: Path, motion_count: int, fixture: bool, preview_scaffold: bool, job: Dict[str, Any], plan_path: Path) -> Dict[str, Any]:
    if not proof:
        return {
            ...
            "issues": ["External BlenderCAM/FabexCNC G-code needs a companion CAM output proof before it can be a production candidate."],
        }
    allowed_schemas = {"hediao3d.blendercam-cam-output-report.v1", "hediao3d.external-cam-output-report.v1"}
    ...
    if str(proof.get("engine") or "").lower() not in {ENGINE, "blendercam", "fabexcnc", "fabex"}:
        issues.append("CAM output proof engine does not match BlenderCAM/FabexCNC")
